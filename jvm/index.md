...
2. 软引用
   ...
   - 仅有软引用引用该对象时，在垃圾回收后，内存仍不足时，会再次出发垃圾回收，回收软引用
...
- 对象首先分配在eden区
- 新生代空间不足时，触发 minor gc，eden区和 from 存活的对象使用 copy 复制到 to 中，存活的
- 对象年龄加 1并且交换 from to
...
#### 串行
 ...
 - 堆内存较小，适合个人电脑
   ...
   **特点：**单线程、简单高效（与其他收集器的单线程相比），采用**复制算法**。对于限定单个CPU的环境来说，Serial收集器由于没有线程交互的开销，专心做垃圾收集自然可以获得最高的单线程回收效率。收集器进行垃圾回收时，必须暂停其他所有的工作线程，直到它结束（Stop The World）
